...
#define WINDOW 50 //窗口大小为5个包
/*
	文件名和文件大小还是单独发送
	根据文件大小，维护一个数组sended[FileSize/SIZEB];
	如果第i个包没有发送和确认接收，sended[i] =0;之外sended[i] =1;
	每次发送时，选择从第一个sended[i]=0处开始，发送WINDOW个数据包
	接收端回复确认信息 recv:j代表在接收端收到了j之前的所有包
	将sended[i:j-1]置1，发送从j开始的WINDOW个包
	为此，我们需要WINDOW个发送线程和1个接收线程。
*/
