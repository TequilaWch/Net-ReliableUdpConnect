...
#define WINDOW 50 //窗口大小为5
/*
	文件名和文件大小还是单独发送
	根据文件大小，维护一个数组recved[FileSize/SIZEB]和一个ruc*数组recvData[FileSize/SIZEB]
	如果第i个包没有确认接收，recved[i] =0;之外recved[i] =1;
	使用WINDOW个接收线程，接收到序号为i的就将其存在对应的recvData中，并将对应的recved置1;
	回发一个recv:j,j是第一个为0的recved
	因此还需要1个发送线程
*/
